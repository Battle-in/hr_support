...
Future<void> _getComplaint(Store<AppState> store) async {
  Response response = await Dio().get('http://localhost:8025/api/validation/getEmployers/');
...
Future<void> _addComplaint(Store<AppState> store, AddComplaintAction action) async {
  Dio().put('http://localhost:8025/api/validation/setEmployers/', data: action.newComplaint.toMap()).then((value) => store.dispatch(GetComplaintAction()));
}

Future<void> _updateComplaint(Store<AppState> store, ChangeComplaintStatusAction action) async {
  Dio().post('http://localhost:8025/api/validation/updateEmployers/', data: action.updateComplaint.toUpdate()).then((value) => store.dispatch(GetComplaintAction()));
}